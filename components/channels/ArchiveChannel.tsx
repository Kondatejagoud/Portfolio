...
export default function ArchiveChannel() {
  ...
  const getStatusColor = (status: ArchiveItem['status']) => {
    switch (status) {
      case 'REPLACED': return 'text-amber-500 border-amber-900 bg-amber-950/20';
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-6 flex flex-col gap-6 font-mono select-none relative">
      ...
      <div className="bg-amber-950/20 border border-amber-900/40 p-2.5 rounded text-amber-500 text-[10px] md:text-xs flex items-center justify-between font-bold z-10">
        <div className="flex items-center gap-2">
          <AlertTriangle size={14} className="text-amber-500 animate-pulse" />
          <span>WARNING: ANALOG BROADCAST DEGRADATION STABILITY WARNING</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start z-10">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-3 rounded border text-xs transition-all duration-200 cursor-pointer flex items-center justify-between group focus:outline-none ${
                    isActive
                      ? 'bg-zinc-950 border-amber-500/80 text-amber-500 shadow-[0_0_6px_rgba(245,158,11,0.15)]'
                      : 'bg-[#0E0E0E] border-zinc-900 hover:border-zinc-850 text-[#777777] hover:text-[#D2D2D2]'
                  }`}
                >
                  <div className="flex flex-col">
                    <span className="text-[9px] uppercase tracking-wider font-bold mb-0.5 text-zinc-600 group-hover:text-amber-500 transition-colors">
                      {item.category}
                    </span>
        ...
        <div className="lg:col-span-8 bg-[#0E0E0E]/90 border border-zinc-800 rounded-lg p-5 flex flex-col gap-6 relative select-text selection:bg-amber-600 selection:text-black">
          ...
          <div className="flex justify-between items-start border-b border-zinc-900 pb-3 flex-wrap gap-2">
            <div className="flex flex-col">
              <span className="text-[9px] text-amber-500 font-bold tracking-widest uppercase">
                ARCHIVED DOSSIER // DEGRADED SIGNAL
              </span>
              <h2 className="text-base md:text-lg font-black text-zinc-300 tracking-wide">
                {selectedItem.title}
              </h2>
            </div>
          ...
          <div className="flex flex-col gap-5 text-xs md:text-sm leading-relaxed text-zinc-400">
            <div className="flex flex-col gap-1">
              <span className="text-[9px] text-[#777777] uppercase tracking-wider font-bold">RELEASE DATE</span>
              <span className="text-zinc-300 font-mono">{selectedItem.date}</span>
            </div>

            <div className="flex flex-col gap-1">
              <span className="text-[9px] text-[#777777] uppercase tracking-wider font-bold">ARCHIVE DESCRIPTION</span>
              <p className="text-zinc-300 font-sans leading-relaxed">{selectedItem.details}</p>
            </div>

            {selectedItem.replacedBy && (
              <div className="flex flex-col gap-1.5 bg-amber-950/10 border border-amber-900/20 p-3 rounded">
            ...
            <div className="flex flex-col gap-1.5 bg-zinc-950 border border-zinc-900 p-3 rounded">
              <span className="text-[9px] text-[#777777] uppercase tracking-wider font-bold">RETROSPECTIVE LESSON</span>
              <p className="text-zinc-300 italic font-sans leading-relaxed">
