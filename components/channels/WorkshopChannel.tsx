    ...
    <div className="flex-1 overflow-y-auto p-4 md:p-6 flex flex-col gap-6 font-mono select-text selection:bg-[#00D9FF] selection:text-black">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-6 flex flex-col gap-6 w-full">
              ...
              currentlyBuilding.map((item, idx) => (
                <div key={idx} className="flex flex-col gap-3">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-green-400 bg-green-950/20 border border-green-900/60 px-2 py-0.5 rounded text-[9px] font-bold animate-pulse">
                      ● {item.status}
                    </span>
                  </div>
          ...
          {/* 5. Things That Broke Section */}
          <section className="flex flex-col gap-3 bg-[#0D1013] border border-zinc-800/60 p-4 rounded-lg">
            <span className="text-[10px] text-red-500 tracking-wider uppercase font-bold flex items-center gap-1.5 border-b border-zinc-900 pb-1.5 animate-pulse">
              <AlertTriangle size={12} />
              THINGS THAT BROKE // TROUBLESHOOTING LOGS
            </span>
            <div className="flex flex-col gap-4">
              {problems.map((prob, idx) => (
                <div key={idx} className="bg-[#080A0C] border border-zinc-900/80 rounded p-3.5 flex flex-col gap-2.5">
          ...
          <section className="flex flex-col gap-3 bg-[#0D1013] border border-zinc-800/60 p-4 rounded-lg select-none">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 md:gap-3 p-3.5 bg-[#080A0C] border border-zinc-900 rounded-lg">
              {buildProcess.map((step, idx) => (
                <React.Fragment key={idx}>
                  <div className="px-2.5 py-1 bg-[#0D1013] border border-zinc-850 rounded font-mono text-[9px] md:text-[10px] text-[#E6E8EA] font-extrabold uppercase select-none">
                    {step}
                  </div>
          ...
          <section className="flex flex-col gap-3 bg-[#0D1013] border border-zinc-800/60 p-4 rounded-lg">
            ...
            <div className="flex flex-col gap-3 max-h-[300px] overflow-y-auto pr-1">
              {developmentLogs.map((log, idx) => (
                <div key={idx} className="bg-[#080A0C] border border-zinc-900/60 rounded p-3 flex flex-col gap-1.5">
                  <div className="flex justify-between items-center border-b border-zinc-900/40 pb-1 flex-wrap gap-2">
                    <span className="text-[9px] text-zinc-500 font-mono">{log.date}</span>
                  ...
                  <div className="flex justify-end pt-1">
                    <span className="text-green-400 bg-green-950/20 px-2 py-0.5 rounded text-[8px] font-bold tracking-widest flex items-center gap-1 uppercase">
                      <CheckCircle2 size={8} />
                      {log.status}
                    </span>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
              {technicalNotes.map((note, idx) => (
                <div key={idx} className="bg-[#080A0C] border border-zinc-900 rounded p-3 flex flex-col gap-2 relative">
                  ...
                  <div className="mt-auto pt-2 border-t border-zinc-900/60 flex justify-between items-center text-[8px] text-zinc-600 font-bold">
